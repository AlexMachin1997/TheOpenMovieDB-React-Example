# Implementation plan: Extract `useDebouncedValue` hook
...
was `D3`; it was written as a single file, so there is no separate `spec.md` or `discovery.md`.
...
([`DebouncableInput.stories.tsx:45,50,105`](../../packages/ui-core/src/components/DebouncableInput/DebouncableInput.stories.tsx)), which is slow and flaky.
...
- [x] One hook owns debounce + sync; neither component re-implements it. — also deleted
      ...
      roadmap README explicitly assigned it to `10`).
